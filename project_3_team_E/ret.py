...
def get_genre(id, genres_df):
    result = genres_df[genres_df['id'] == id]['genre'].values
    if len(result) > 0:
        return set(result[0].replace("[", "").replace("]", "").replace("'", "").split(', '))
    else:
        return set()
...
'''
function to generate a cosine similarity matrix
'''
def generate_cos_sim_matrix(embedding):
    #embedding.drop(7298, axis=0, inplace=True)
    ...
    print(embedding_transpose.shape) # (1328, 10093)
